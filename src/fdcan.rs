...
use crate::pac::{
    FDCAN_MSGRAM, FDCAN_MSGRAM_LEN_WORDS, FDCAN1_REGISTER_BLOCK_ADDR, FDCAN2_REGISTER_BLOCK_ADDR,
    RCC_REGISTER_BLOCK_ADDR,
};
use crate::{CLOCK_DOMAIN_SYNCHRONIZATION_DELAY, pac};
use core::marker::PhantomData;

pub struct FdCan<M> {
    pub(crate) regs: pac::registers::Fdcan,
    #[cfg(feature = "embassy")]
...
/// Create FDCAN1 instance. This method can be called only once, otherwise Error::PeripheralTaken is returned.
/// Peripheral is disabled via RCC.
#[cfg(feature = "g0")]
pub fn new_fdcan1() -> Result<FdCan<PoweredDownMode>, Error> {
    #[cfg(feature = "embassy")]
    let state = crate::embassy::state_fdcan1()?;
...
impl FdCanInstances {
    /// Creates FDCAN instances in powered down state (enable flag is cleared in RCC as well).
