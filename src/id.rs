...
impl StandardId {
    ...
    /// # Safety
    ///
    /// The caller must ensure that `raw` is in the valid range, otherwise frame with an incorrect ID will be sent or
    /// incorrect filter will be created.
    #[inline]
...
impl Id {
    pub(crate) fn reg_value(&self) -> u32 {
        const STANDARD_SHIFT: u32 = 18;
        match self {
            Id::Standard(sid) => (sid.0 as u32) << STANDARD_SHIFT,
            Id::Extended(eid) => eid.0,
        }
    }
}
...
impl From<Id> for Xtd {
    #[inline]
    fn from(id: Id) -> Self {
        match id {
            Id::Standard(_) => Xtd::ElevenBits,
            Id::Extended(_) => Xtd::TwentyNineBits,
        }
    }
}
...
///
/// Can be either a standard identifier (11bit, range: 0..=0x7FF) or an
/// extendended one (29bit, range: 0..=0x1FFFFFFF).
///
...
impl IdReg {
    ...
    pub fn to_id(self) -> Id {
        if self.is_extended() {
            Id::Extended(unsafe {
                ExtendedId::new_unchecked((self.0 & Self::EXTENDED_MASK) >> Self::EXTENDED_SHIFT)
            })
        } else {
            Id::Standard(unsafe {
